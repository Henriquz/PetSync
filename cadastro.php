...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nome || !$email || !$senha) {
        ...
        if ($chk->get_result()->num_rows) {
            ...
            if ($ins->execute()) {
                $ok = 'Cadastro realizado! Você já pode fazer login.';
            } else {
                $erro = 'Erro MySQL: ' . $ins->error;
            }
        }
    }
}
...
<html lang="pt-BR"><head>
  <meta charset="UTF-8">
  <title>Cadastro – PetSync</title>
  <link rel="stylesheet" href="assets/css/749565270a375176.ltr.css">
        ...
        <div class="container mx-auto px-4">
            <div class="text-center">
                ...
                <p class="text-lg text-gray-600 max-w-2xl mx-auto">
                    Cadastre-se para acessar ofertas exclusivas, acompanhar seus pedidos e aproveitar todos os benefícios de ser um cliente PetSync.
                </p>
            </div>
        </div>
    </section>
            ...
            <div class="max-w-md mx-auto">
                <div class="bg-white rounded-lg shadow-lg overflow-hidden auth-card">
                    ...
                    <div class="p-6">
                        <!-- mensagens PHP -->
                        <?php if ($erro): ?>
                          <div id="msg-box" class="bg-red-100 text-red-700 px-3 py-2 rounded mb-4"><?= $erro ?></div>
                        <?php elseif ($ok): ?>
                          <div id="msg-box" class="bg-green-100 text-green-700 px-3 py-2 rounded mb-4"><?= $ok ?></div>
                        <?php else: ?>
                          <div id="msg-box" class="hidden"></div>  <!-- fica invisível até o JS usar -->
                        <?php endif; ?>
                        ...
                        <form id="register-form"
                          ...
                          <div>
                            ...
                            <input name="nome"
                                   type="text"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-petBlue focus:outline-none"
                          ...
                          <!-- Botão -->
                          <button type="submit"
                                  class="w-full bg-petOrange hover:bg-orange-600 text-white font-semibold py-2 rounded-lg transition">
                            Criar Conta
                          </button>
                        </form>
                        ...
                        <div class="mt-6 text-center">
                            <p class="text-gray-600">
                                Já tem uma conta? <a href="login.html" class="text-petBlue hover:underline">Entrar</a>
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
  ...
  document.getElementById('mobile-menu-button')
    ?.addEventListener('click',()=>document.getElementById('mobile-menu')
...
/* confirma senha no cliente */
const form   = document.getElementById('register-form');
const senha  = document.getElementById('senha');
const sConf  = document.getElementById('senha_conf');
const box    = document.getElementById('msg-box');

form.addEventListener('submit', e=>{
  if (senha.value !== sConf.value) {
    e.preventDefault();
    /* mostra caixa vermelha como nos outros erros */
    box.textContent = 'As senhas não coincidem!';
